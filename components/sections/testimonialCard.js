...
const testimonialCard = ({ bigCard, containerClassName, image, name, rating, testimony, testimony2 }) => {
  const paragraphsStyle =
    'mt-1 max-w-[420px] xl:max-w-[480px] dark:text-white text-[0.875rem] text-amcovad-secondary-600 font-Inter font-normal';
  return (
    <div
      ...
      <div className="flex items-start text-left">
        ...
        <div className="ml-6">
          <p className="flex items-baseline">
            <span className="text-amcovad-secondary-300 dark:text-gray-200 font-bold">{name}</span>
          </p>
          <div className="flex items-center mt-1">
            <span className="text-gray-500 dark:text-gray-300  pr-1 text-sm">{rating}</span>
