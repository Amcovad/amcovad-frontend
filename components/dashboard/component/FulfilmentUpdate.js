...
const FulfilmentUpdate = ({ buttonColor, name, quantity, status, userImage }) => {
  return (
    <li className="flex items-center justify-between my-3 space-x-2 text-sm text-amcovad-black font-normal">
      <div className="flex items-center">
        <Image src={userImage} width={48} height={48} alt="user image" className="mx-auto object-cover rounded-xl " />
        <p className="text-sm text-amcovad-black font-normal pl-3 ">{name}</p>
      </div>
      <div>
        <p>{quantity}</p>
      </div>
      <p
        className={classNames(
          'px-3 py-1 flex items-center text-sm rounded-lg font-Poppins text-amcovad-white',
          buttonColor
        )}
      >
        {status}
