...
import {
  createData,
  fulfilmentUpdateData,
  transactionCardData,
  fulfilmentTimelineData,
  statusData
} from '@/data/dashboard/index';
import {
  ColoredCard,
  CreateCard,
  FulfilmentUpdate,
...
function Transaction() {
  ...
  return (
    <>
      <DashboardLayout title="Dashboard" showAlertHere={showAlert && <Alert onClick={() => setShowAlert(false)} />}>
        <main className="h-screen w-full pb-36 pt-4 px-2 lg:px-4 lg:pt-2">
          <div className="flex flex-wrap">
            <div className="w-full mb-16 ">
              <div className="flex flex-wrap">
                {createData.map(({ backgroundColor, icon, title, url }, index) => {
                  return (
                    <CreateCard
                      key={index}
                      threeColumn
